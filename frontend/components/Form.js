...
function Form(props) {
  ...
  return (
    <form id="form" onSubmit={onSubmit}>
      ...
      <button disabled={props.form.newQuestion.trim().length <= 1 || props.form.newTrueAnswer.trim().length <= 1 || props.form.newFalseAnswer.trim().length <= 1} id="submitNewQuizBtn">Submit new quiz</button>
    </form>
  )
}
